...
const { cyan, yellow, green, gray, red } = require('picocolors')
...
function summary() {
    ...
    const res = sorted.map(([name, val]) => {
        return [
            name,
            !val ? red('FAILED') : ms(val / 1e6, { formatSubMilliseconds: true, compact: true }),
            `${((val / averageLoadTime) * 100).toFixed(2)}%`
        ]
    })

    // reveal results
    console.log('\x1b[K' + table(res, { align: ['l', 'l', 'r'], hsep: cyan(' | '), stringLength: (str) => str.replace(ansiRegex(), '').length }))
    ...
    try {
        if (useESM || isESM(modulePath)) {
            ...
            return new Promise((resolve, reject) => {
                ...
                child.on('message', ({ timespan, error }) => {
                    if (error) {
